...
func (stub *stub) register(ctx context.Context) error {
	...
	stub.logger.Infof(ctx, "Registering plugin %s (with NRI version %s)...",
...
func (stub *stub) Configure(ctx context.Context, req *api.ConfigureRequest) (rpl *api.ConfigureResponse, retErr error) {
	...
	stub.logger.Infof(ctx, "Configuring plugin %s for runtime %s/%s (with NRI version %s)...",
		stub.Name(), req.RuntimeName, req.RuntimeVersion, req.NRIVersion)

	if req.NRIVersion == "" || req.NRIVersion == api.DevelVersion {
		nv, err := api.InferVersionFromRuntime(req.RuntimeName, req.RuntimeVersion)
		if err != nil {
			stub.logger.Errorf(ctx, "failed to infer NRI version: %v", err)
		} else {
			stub.logger.Infof(ctx, "infered NRI version: %s", nv)
			req.NRIVersion = nv
		}
	}
