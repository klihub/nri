...
func (p *plugin) synchronize(ctx context.Context, pods []*PodSandbox, containers []*Container) ([]*ContainerUpdate, error) {
	...
	for {
		...
		if err == nil {
			...
			podsPerMsg, ctrsPerMsg, err = recalcObjsPerSyncMsg(ctx, podsPerMsg, ctrsPerMsg, err)
			if err != nil {
				p.close()
				return nil, err
			}
...
func recalcObjsPerSyncMsg(ctx context.Context, pods, ctrs int, err error) (int, int, error) {
	const (
		minObjsPerMsg = 8
	)
	...
	if pods+ctrs <= minObjsPerMsg {
		return pods, ctrs, fmt.Errorf("failed to syncronize plugin with split messages")
	}
