...
var _ = Describe("Plugin container creation adjustments", func() {
	...
	When("there are restrictions", func() {
		...
		DescribeTable("restricted adjustments should fail to",
			func(subject string, restricted bool, expected *api.ContainerAdjustment) {
				var (
					runtime = s.runtime
					plugins = s.plugins
					ctx     = context.Background()
				...
				if restricted {
					...
					Expect(plugins[0].Wait(PluginDisconnected, timeout)).To(Succeed())
				}
			},

			Entry("adjust annotations", "annotation", false,
				&api.ContainerAdjustment{
					Annotations: map[string]string{
						"key": "10-foo",
					},
