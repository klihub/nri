...
import (
	...
	semver "github.com/containerd/nri/pkg/version"
)

const (
	UnknownVersion = semver.UnknownVersion
	DevelVersion   = semver.DevelVersion
)

// InferVersionFromRuntime tries to infer the version of NRI used in the runtime.
func InferVersionFromRuntime(runtime, version string) (string, error) {
	var (
		unknown    = semver.UnknownVersion
	...
	tag, err := semver.LastStableTagged(version)
	if err != nil {
		return unknown, err
	}
	...
	if nriVersion == "" {
		return unknown, fmt.Errorf("unknown runtime version %q %q", runtime, version)
	}
