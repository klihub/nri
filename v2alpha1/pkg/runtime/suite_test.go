...
func (m *mockRuntime) createPodAndContainer(ctx context.Context, pod *api.PodSandbox, ctr *api.Container) error {
	err := m.runtime.RunPodSandbox(ctx, &api.StateChangeEvent{
		Pod: pod,
	})
	if err != nil {
...
var (
	PluginStarted      = &Event{Type: Started}
...
func (q *EventQ) Wait(w *Event, deadline <-chan time.Time) (*Event, error) {
	...
	if q.c != nil {
		return nil, errors.New("EventQ already busy Wait()ing...")
	}
	...
	for {
		...
		case _ = <-deadline:
			return nil, errors.Errorf("EventQ timed out Wait()ing for %s...", w)
		}
	}
}
