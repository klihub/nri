...
var _ = Describe("Configuration", func() {
	...
	When("invalid", func() {
		var (
			invalid_config = "xyzzy gibberish foobar"
		)

		BeforeEach(func() {
			s.Prepare(invalid_config, &mockRuntime{})
		})
	...
	When("empty", func() {
		...
		BeforeEach(func() {
			s.Prepare(empty_config, &mockRuntime{}, &mockPlugin{idx: "00", name: "test"})
		})
	...
	When("external connections are explicitly disabled", func() {
		var (
			config = "disablePluginConnections: true"
		)
	...
	When("external connections are explicitly enabled", func() {
		var (
			config = "disablePluginConnections: false"
		)
		...
		DescribeTable("should honor plugins' event subscriptions",
			func(subscriptions ...string) {
				var (
					...
					order       []*mockPlugin
					recordOrder = func(p *mockPlugin, pod *api.PodSandbox, ctr *api.Container) {
...
var _ = Describe("Plugin container creation adjustments", func() {
	...
	When("there are multiple plugins", func() {
		...
		DescribeTable("should be successfully combined if there are no conflicts",
			...
			Entry("adjust labels (conflicts)", "label", false, true, nil),
			Entry("adjust labels", "label", true, false,
				&api.ContainerAdjustment{
					Labels: map[string]string{
						"-key": "",
						"key":  "10-foo",
...
var _ = Describe("Plugin container updates during creation", func() {
	...
	When("there is a single plugin", func() {
		...
		DescribeTable("should be successfully collected without conflicts",
			...
			Entry("adjust memory resources", "resources/memory",
				&api.ContainerUpdate{
					Linux: &api.LinuxContainerUpdate{
						Resources: &api.LinuxResources{
							Memory: &api.LinuxMemory{
	...
	When("there are multiple plugins", func() {
		...
		DescribeTable("should fail with conflicts, successfully collected otherwise",
			...
			Entry("adjust CPU resources", "resources/cpu", "both", nil),
			Entry("adjust CPU resources", "resources/cpu", "10-foo",
				&api.ContainerUpdate{
					Linux: &api.LinuxContainerUpdate{
						Resources: &api.LinuxResources{
							Cpu: &api.LinuxCPU{
			...
			Entry("adjust memory resources", "resources/memory", "both", nil),
			Entry("adjust memory resources", "resources/memory", "10-foo",
				&api.ContainerUpdate{
					Linux: &api.LinuxContainerUpdate{
						Resources: &api.LinuxResources{
							Memory: &api.LinuxMemory{
			...
			Entry("adjust class-based resources", "resources/classes", "both", nil),
			Entry("adjust class-based resources", "resources/classes", "10-foo",
				&api.ContainerUpdate{
					Linux: &api.LinuxContainerUpdate{
						Resources: &api.LinuxResources{
			...
			Entry("adjust hugepage limits", "resources/hugepagelimits", "both", nil),
			Entry("adjust hugepage limits", "resources/hugepagelimits", "10-foo",
				&api.ContainerUpdate{
					Linux: &api.LinuxContainerUpdate{
						Resources: &api.LinuxResources{
							HugepageLimits: []*api.HugepageLimit{
...
func stripLabels(a *api.ContainerAdjustment) {
	if len(a.Labels) == 0 {
		a.Labels = nil
	}
}
