...
func emulateWithPreConnect() (SocketPair, error) {
	...
	if err != nil {
		return invalid, errors.Wrap(err, "failed to emulate socketpair (local Socket())")
	}
	defer func() {
		if err != nil {
			sys.CloseHandle(l)
	...
	err = sys.Bind(l, &sa)
	if err != nil {
		return invalid, errors.Wrap(err, "failed to emulate socketpair (Bind())")
	}

	/*sn, err = sys.Getsockname(l)
	if err != nil {
		return invalid, errors.Wrap(err, "failed to emulate socketpair (Getsockname())")
	}*/

	p, err = socket(sys.AF_INET, sys.SOCK_STREAM, 0)
	if err != nil {
		return invalid, errors.Wrap(err, "failed to emulate socketpair (peer Socket())")
	}
	...
	err = sys.Listen(l, 0)
	if err != nil {
		return invalid, errors.Wrap(err, "failed to emulate socketpair (Listen())")
	}
	...
	if err != nil {
		return invalid, errors.Wrap(err, "failed to emualte socketpair (Accept())")
	}
	defer func() {
		if err != nil {
			sys.CloseHandle(a)
...
func (sp SocketPair) LocalConn() (net.Conn, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err,
			"failed to create net.Conn for %s[0]", sp.fileName())
	}
	return conn, nil
}
...
func (sp SocketPair) PeerConn() (net.Conn, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err,
			"failed to create net.Conn for %s[1]", sp.fileName())
	}
	return conn, nil
}
...
func accept(l sys.Handle, domain, typ, proto int) (sys.Handle, error) {
	...
	if err != nil {
		return sys.InvalidHandle, errors.Wrap(err, "failed to emulate socketpair (accept)")
	}

	err = sys.AcceptEx(l, a, (*byte)(unsafe.Pointer(&buf)), 0, cnt, cnt, &cnt, &overlap)
	if err != nil {
		return sys.InvalidHandle, errors.Wrap(err, "failed to emulate socketpair (AcceptEx())")
	}
