...
type ConfigureInterface interface {
	...
	Configure(string) (api.EventMask, error)
}
...
type handlers struct {
	Configure           func(string) (api.EventMask, error)
...
func New(p interface{}, opts ...Option) (Stub, error) {
	...
	// if plugin name and index are not given, get them from binary name
	if stub.name == "" {
		name := filepath.Base(os.Args[0])
		idx, base, err := api.ParsePlugin(name)
		if err == nil {
			stub.idx, stub.name = idx, base
		} else {
			stub.name = name
		}
	}
	...
	// connect to NRI (if we're not preconnected) and set up multiplexing
	if stub.conn == nil {
	...
	// create ttrpc (NRI Plugin) server
	...
	// create ttrpc (NRI Runtime) client
...
func (stub *stub) Stop() error {
	...
	log.Infof(noCtx, "Stopping plugin %ss...", stub.Name())
