...
func (p *plugin) CreateContainer(pod *api.PodSandbox, ctr *api.Container) (*api.ContainerAdjustment, []*api.ContainerUpdate, error) {
	...
	if pod.GetName() == podName {
		cgroupPath := "/sys/fs/cgroup" + pod.GetLinux().GetCgroupParent() + "/cpu.weight"

		log.Infof("- overriding CPU weight to %s (in %s)...", cpuWeight, cgroupPath)
		hook := &api.Hook{
			Path: "/bin/bash",
			Args: []string{
				"/bin/bash",
				"-c",
				"echo " + cpuWeight + " > " + cgroupPath,
			},
		}
...
func main() {
	...
	flag.StringVar(&podName, "pod", "test-pod", "name of pod to overwrite cpu.weight for")
	flag.StringVar(&cpuWeight, "cpu-weight", "750", "CPU weight to use")
	flag.Parse()
